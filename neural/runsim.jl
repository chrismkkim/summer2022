...
function runsim(p,w0Index,w0Weights,nc0,wpIndexOut,wpWeightOut,ncpOut,stim,rollingavg,matchedCells)
...
Nexam = 10
vsum = zeros(Nsteps, Nexam)
vbalE = zeros(Nsteps, Nexam)
vbalI = zeros(Nsteps, Nexam)
vplasE = zeros(Nsteps, Nexam)
vplasI = zeros(Nsteps, Nexam)
idx = 1
    ...
    for ci = 1:Ncells
        ...
        if rollingavg 
            if t > Int(p.stim_off) && t <= Int(p.train_time) && mod(t,1.0) == 0
                ...
                # xeplastic[:,ci], xeplasticcnt = funRollingAvg(p,t,wid,widInc,learn_nsteps,xeplastic[:,ci],xeplasticcnt,xpedecay[ci],ci)
                # xiplastic[:,ci], xiplasticcnt = funRollingAvg(p,t,wid,widInc,learn_nsteps,xiplastic[:,ci],xiplasticcnt,xpidecay[ci],ci)
            end
        end

        if ci in matchedCells[1:Nexam]
            vsum[ti,idx] = synInput + mu[ci]
            vbalE[ti,idx] = xedecay[ci] + mu[ci]
            vbalI[ti,idx] = xidecay[ci]
            vplasE[ti,idx] = xpedecay[ci]
            vplasI[ti,idx] = xpidecay[ci]
            if idx == Nexam
                idx = 1
            else 
                idx += 1
...
for k = 1:learn_nsteps
    ...
    # xeplastic[k,:] = xeplastic[k,:]/xeplasticcnt[k]
    # xiplastic[k,:] = xiplastic[k,:]/xiplasticcnt[k]
end

return xsum, times, ns, vsum, vbalE, vbalI, vplasE, vplasI
