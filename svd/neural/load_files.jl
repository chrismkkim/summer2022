function load_files(dirData, lickRL)
    ...
    if lickRL == "right"
        ...
        # fname_ffwdRate = dirData * "ffwdRate_R.jld"
    elseif lickRL == "left"
        fname_stim = dirData * "stim_L.jld"
        # fname_ffwdRate = dirData * "ffwdRate_L.jld"
    end
    fname_ffwdRate = dirData * "ffwdRate.jld"
    fname_wpWeightFfwd = dirData * "wpWeightFfwd_loop$(nloop).jld"
    ...
    # fname_wpWeightIn = dirData * "wpWeightIn_loop$(200).jld"
    # fname_wpWeightOut = dirData * "wpWeightOut.jld"
    fname_wpWeightOut = dirData * "wpWeightOut_loop$(nloop).jld"
    ...
    return p, w0Index, w0Weights, nc0, wpIndexIn, wpIndexOut, wpIndexConvert, wpWeightOut, ncpIn, ncpOut, stim, almOrd, matchedCells, ffwdRate, wpWeightFfwd
