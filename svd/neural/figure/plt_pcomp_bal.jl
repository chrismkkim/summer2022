function plt_pcomp_bal(dirfig, fname, startind, Mbal)
    ...
    for ii = 1:ncomp
        ...
        plot(timev_exc, Mbal.proj[:,ii], c="gray", lw=0.8)
    ...
    ncomp = 9
    expvar_exc = cumsum(principalvars(Mbal))[1:ncomp] / tvar(Mbal)
    ...
    plot(collect(1:ncomp), expvar_exc, color="gray", marker="o", lw=1.1, ms=3, mec="None", clip_on=false)
    ...
    legend_elements = [Line2D([0], [0], color="gray", lw=0, label="Null")]
    legend(handles=legend_elements, frameon=false, fontsize=7, handlelength=0, loc=1, labelcolor="linecolor")     
