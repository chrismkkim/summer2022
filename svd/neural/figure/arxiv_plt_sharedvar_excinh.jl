function plt_sharedvar_excinh(fname, svar_cor, svarX, svarY, svarX_ksum, svarX_tsum, svarY_ksum, svarY_tsum)
    ...
    for ii = 1:ncomp
        subplot(2,2,ii)
        plot(timev, sgn[ii]*svarX[2:end,ii], c="red")
        plot(timev, sgn[ii]*svarY[2:end,ii], c="blue")
    ...
    ax = fig.add_subplot(111)
    xlabel("Exc", fontsize=12, labelpad=30)
    ylabel("Inh", fontsize=12, labelpad=30)
    ax.set_frame_on(false)
    ax.tick_params(left=false, bottom=false)
    ax.tick_params(labelleft=false, labelbottom=false)
    ...
    figure(figsize=(3.5,3))
    ncomp = 10
    plot(collect(1:ncomp), svarX_ksum[1:ncomp]/svarX_tsum, c="red", marker="o")
    plot(collect(1:ncomp), svarY_ksum[1:ncomp]/svarY_tsum, c="blue", marker="o")
    ylim([5e-4,1])
    yscale("log")
    xlabel("SVC", fontsize=12)
    ...
    figure(figsize=(3.5,3))
    ncomp = 10
    plot(collect(1:ncomp), cumsum(svarX_ksum[1:ncomp]/svarX_tsum), c="red", marker="o")
    plot(collect(1:ncomp), cumsum(svarY_ksum[1:ncomp]/svarY_tsum), c="blue", marker="o")
    legend_elements = [Line2D([0], [0], color="red", lw=2, label="Exc"),
                       Line2D([0], [0], color="blue", lw=2, label="Inh")]
