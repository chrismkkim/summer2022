...
function runsim(p,w0Index,w0Weights,nc0,wpIndexOut,wpWeightOut,ncpOut,stim,ffwdRate,wpWeightFfwd)
...
forwardInputsXPrev = zeros(Ncells)
...
ubal = zeros(Nsteps,10)
uebal = zeros(Nsteps,10)
uibal = zeros(Nsteps,10)
uplas = zeros(Nsteps,10)
uext = zeros(Nsteps,10)

Ntrain = (p.train_time - p.stim_off)/p.dt
ubal_avg = zeros(Ncells)
uebal_avg = zeros(Ncells)
uibal_avg = zeros(Ncells)
uplas_avg = zeros(Ncells)
uext_avg = zeros(Ncells)
...
for ti=1:Nsteps
    ...
    for ci = 1:Ncells
        ...
        xxdecay[ci] += -dt*xxdecay[ci]/taudecay_plastic + forwardInputsXPrev[ci]/taudecay_plastic
        synInputBalanced[ci] = xedecay[ci] + xidecay[ci]
        synInput = synInputBalanced[ci] + xpdecay[ci] + xxdecay[ci]
        
        if ci <= 10
            ubal[ti,ci] = synInputBalanced[ci] + mu[ci]
            uebal[ti,ci] = xedecay[ci] + mu[ci]
            uibal[ti,ci] = xidecay[ci]
            uplas[ti,ci] = xpdecay[ci]
            uext[ti,ci] = xxdecay[ci]
        end

        if t > p.stim_off 
            ubal_avg[ci] += (synInputBalanced[ci] + mu[ci]) / Ntrain
            uebal_avg[ci] += (xedecay[ci] + mu[ci]) / Ntrain
            uibal_avg[ci] += xidecay[ci] / Ntrain
            uplas_avg[ci] += xpdecay[ci] / Ntrain
            uext_avg[ci] += xxdecay[ci] / Ntrain
        end
    ...
    if ti > Int(stim_off/dt)
        ...
        for ci = 1:p.Lffwd
            ...
            if rndFfwd[ci] < ffwdRate[tidx,ci]/(1000/p.dt)
                ...
                for j = 1:Ne
                    forwardInputsX[j] += wpWeightFfwd[j,ci]
                end #end loop over synaptic projections
            end #end if spiked
    ...
    forwardInputsXPrev = copy(forwardInputsX)
...
return ubal_avg, uebal_avg, uibal_avg, uplas_avg, uext_avg, ubal, uebal, uibal, uplas, uext, times, ns
