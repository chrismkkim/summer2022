function sim_perturbation_lickLeft(dirdata, rate_R_avg, rate_L_avg, pyr_R, pyr_L, pert_on, pert_off, subpoptype)
    ...
    codingdim_Lp_list = zeros(nsession, ntrial, 151)
    codingdim_R_list = zeros(nsession, ntrial, 151)
    codingdim_L_list = zeros(nsession, ntrial, 151)

    homdim_Lp_list = zeros(nsession, ntrial, 151)
    homdim_L_list = zeros(nsession, ntrial, 151)

    codingdimInh_Lp_list = zeros(nsession, ntrial, 151)
    ...
    for repi = 1:ntrial
        ...
        usum_L_perturb, times_L_perturb, ns_L_perturb = runtestperturb_lickLeft(p,w0Index,w0Weights,nc0,wpIndexOut,wpWeightOut,ncpOut,stim_R,stim_L,ffwdRate[2],wpWeightFfwd[2],isperturbed, pert_on, pert_off)
        ...
        for popi = 1:nsession
            ...
            rate_L_perturb = spk2rate(p, matchedCells_subpop, times_L_perturb, ns_L_perturb)
            ...
            pyr_codingdim = mean(rate_L_avg - rate_R_avg, dims=1)[matchedCells_subpop]
            ...
            codingdim_Lp_list[popi, repi,:] = mean((rate_L_perturb .- reshape(rate_M,1,:)).*reshape(pyr_codingdim,1,:), dims=2)[:]

            # homogeneous dimension --- excitatory
            pyr_homdim = ones(size(rate_R)[2])
            pyr_homdim = pyr_homdim / sqrt(mean(pyr_homdim.^2))

            homdim_Lp_list[popi, repi,:] = mean((rate_L_perturb .- reshape(rate_M,1,:)).*reshape(pyr_homdim,1,:), dims=2)[:]
            homdim_L_list[popi, repi,:] = mean((rate_L .- reshape(rate_M,1,:)).*reshape(pyr_homdim,1,:), dims=2)[:]
            
            # coding dimension --- untrained inhibitory
            rateInh_L_perturb = spk2rate(p, inhCells, times_L_perturb, ns_L_perturb)
            ...
            fs_codingdim = mean(rate_L_avg - rate_R_avg, dims=1)[inhCells]
            fs_codingdim = fs_codingdim / sqrt(mean(fs_codingdim.^2))

            codingdimInh_Lp_list[popi, repi, :] = mean((rateInh_L_perturb .- reshape(rateInh_M,1,:)).*reshape(fs_codingdim,1,:), dims=2)[:]
    ...
    # excitatory
    codingdim_Lp_mean = dropdims(mean(codingdim_Lp_list, dims=2), dims=2)
    ...
    homdim_Lp_mean = dropdims(mean(homdim_Lp_list, dims=2), dims=2)
    homdim_L_mean = dropdims(mean(homdim_L_list, dims=2), dims=2)

    # inhibitory
    codingdimInh_Lp_list = dropdims(mean(codingdimInh_Lp_list, dims=2), dims=2)
    ...
    return codingdim_R_mean, codingdim_L_mean, codingdim_Lp_mean, homdim_L_mean, homdim_Lp_mean, codingdimInh_R_mean, codingdimInh_L_mean, codingdimInh_Lp_list,
    codingdim_Lp_list, codingdim_L_list, homdim_Lp_list, homdim_L_list
