...
function runtestperturb(p,w0Index,w0Weights,nc0,wpIndexOut,wpWeightOut,ncpOut,stim,ffwdRate,wpWeightFfwd, isperturbed, pert_on, pert_off)
    ...
    for ci = 1:Ncells
        ...
        if isperturbed
            ...
            if t > pert_on && t < pert_on + Int(stim_off - stim_on)
                bias[ci] = mu[ci] + stim[ti-Int(pert_on/dt),ci]
            end
