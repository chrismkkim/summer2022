...
function runtestperturb_lickLeft(p,w0Index,w0Weights,nc0,wpIndexOut,wpWeightOut,ncpOut,stim_R,stim_L,ffwdRate,wpWeightFfwd, isperturbed, pert_on, pert_off)
...
perturb = 6.0*(rand(Ni) .- 0.5) # perturb inhibitory neurons
...
for ti=1:Nsteps
    ...
    for ci = 1:Ncells
        ...
        if t > Int(stim_on) && t < Int(stim_off)
            bias[ci] = mu[ci] + stim_L[ti-Int(stim_on/dt),ci]
        else
            bias[ci] = mu[ci]
        end
        ...
        if isperturbed
            ...
            # # perturb inhibitory neurons
            # if ci > Ne && t > pert_on && t < pert_off
            #     bias[ci] += perturb[ci-Ne] * sin(2*pi*(t/100 - rndphase[ci]))
            # end

            # # perturb exc / inh neurons
            # if t > pert_on && t < pert_off
            #     bias[ci] += perturb[ci] * sin(2*pi*(t/100 - rndphase[ci]))
            # end

            # perturb using stimulus that triggers learned activity
            if t > pert_on && t < pert_on + Int(stim_off - stim_on)
                bias[ci] = mu[ci] + stim_R[ti-Int(pert_on/dt),ci]
            end
