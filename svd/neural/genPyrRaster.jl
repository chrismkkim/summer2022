function genPyrRaster(rate_R, rate_L, cell_R, cell_L, units_R, units_L, spiketime_R, spiketime_L, eventtime_R, eventtime_L)
    ...
    times_R = zeros(length(cellsOrd), 200)
    ns_L = zeros(Int, length(cellsOrd))
    times_L = zeros(length(cellsOrd), 200)
    triali = 2
