...
fname_rate_navg = dirsim * "rate_navg$(navg).jld"
rate_navg = load(fname_rate_navg, "rate")[3:100,:,1]
...
idx_pyr = .!isnan.(cor(rtargFS, rtargPyr, dims=1)[1,:])
...
# Pyr and trained exc
cor_pyrexc = cor(rtargPyr_trim, rateExc, dims=1)
...
# FS and trained inh
cor_fsinh = cor(rtargFS_trim, rateInh, dims=1)
...
svar_pyrexc_Pyr_tsum, svar_pyrexc_Exc_tsum = sharedVar(svd_pyrexc, rtargPyr_trim, rateExc)
...
svar_fsinh_FS_tsum, svar_fsinh_INH_tsum = sharedVar(svd_fsinh, rtargFS_trim, rateInh)
...
svar_balanced_Exc_ksum, svar_balanced_Exc_tsum)
